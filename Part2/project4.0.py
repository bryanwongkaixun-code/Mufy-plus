...
# Try to import the Google GenAI SDK safely
try:
...
# ------------------------------------------------------------------
# 📂 2. DATA STORAGE HANDLERS WITH FAIL-SAFES
# ------------------------------------------------------------------
def load_saved_data():
    try:
        ...
        st.warning(f"⚠️ App notice: Resetting file storage data ({data_err})")
...
def save_data_to_file():
    try:
        ...
        st.error(f"❌ Server storage write failure: {write_err}")
...
# ------------------------------------------------------------------
# 🤖 3. INTELLIGENT GEMINI API ASSISTANT (Fail-Safe Integration)
# ------------------------------------------------------------------
...
if ai_prompt:
    if not SDK_AVAILABLE:
        st.error("❌ The required library `google-genai` is not installed on the server. Please check your requirements.txt file!")
    else:
        try:
            # 🔐 Safe Secret Retrieval setup to bypass public cloud crashes
            if "MY_PRIVATE_API_KEY" in st.secrets:
                MY_PRIVATE_API_KEY = st.secrets["MY_PRIVATE_API_KEY"]
            else:
                # Local development fallback
                MY_PRIVATE_API_KEY = "PASTE_YOUR_LOCAL_KEY_HERE_IF_NOT_ON_CLOUD"
            
            if MY_PRIVATE_API_KEY == "PASTE_YOUR_LOCAL_KEY_HERE_IF_NOT_ON_CLOUD":
                st.warning("⚠️ Setup needed: Add your API key in Streamlit Cloud Secrets under the name MY_PRIVATE_API_KEY.")
            else:
                ...
                with st.spinner("Gemini is analyzing the seating chart..."):
                    try:
                        # Attempt 1: Core Gemini 2.5 Flash model
                        ...
                        # Fallback Attempt 2: Route to 2.5-pro if 2.5-flash has traffic spikes
            ...
            if "503" in str(e) or "UNAVAILABLE" in str(e):
                ...
                st.error(f"❌ Connection or Authentication Failure: {e}")
st.markdown("---")
...
# --- OPTION 2: CANCEL A BOOKING ---
    ...
    if st.button("Cancel Reservation", type="primary"):
        ...
        if st.session_state.seats[r][c] == 'O':
            ...
            st.rerun() datetime import datetime
from google import genai
